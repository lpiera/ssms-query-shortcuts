/* table size name */ declare @a table (id int, rc bigint, rs bigint, us bigint, dt bigint) 
...
select case when rn = 1 then db_name() else '' end db,
  ...
  from (select top 100 percent row_number() over(order by object_schema_name(id), object_name(id)) rn,
